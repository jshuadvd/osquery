...
void Client::encryptConnection() {
  ...
  if (client_options_.always_verify_peer_) {
    if (!client_options_.sni_hostname_) {
      throw std::logic_error("The SNI hostname is missing from the Client options");
    }

    const auto &sni_hostname = client_options_.sni_hostname_.value();
    ssl_sock_->set_verify_callback(boost::asio::ssl::rfc2818_verification(sni_hostname));
  }
